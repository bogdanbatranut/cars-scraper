...
type MercedesBenzRONamingMapper struct {
	fuelTypes    map[string]CodesTextEntry
	salesClasses map[string]CodesTextEntry
	bodyGroups   map[string]CodesTextEntry
}

func NewMercedesBenzRoNamingMapper() *MercedesBenzRONamingMapper {
	return &MercedesBenzRONamingMapper{
		fuelTypes:    initFuels(),
...
func (mapper MercedesBenzRONamingMapper) GetBrand(brand string) *string {
	return nil
}

func (mapper MercedesBenzRONamingMapper) GetModelCode(model string) *string {
	return nil
}

func (mapper MercedesBenzRONamingMapper) GetModelText(model string) *string {
	return nil
}

func (mapper MercedesBenzRONamingMapper) GetFuelCode(fuel string) *string {
	return nil
}

func (mapper MercedesBenzRONamingMapper) GetFuelText(fuel string) *string {
	return nil
}

func (mapper MercedesBenzRONamingMapper) GetEngineTypeCodesTextEntry(fuel string) CodesTextEntry {
	return mapper.fuelTypes[fuel]
}

func (mapper MercedesBenzRONamingMapper) GetSalesClassCodesTextEntry(model string) CodesTextEntry {
	return mapper.fuelTypes[model]
}

func (mapper MercedesBenzRONamingMapper) GetMarketCriteria(criteria jobs.Criteria) Criteria {
...
func (mapper MercedesBenzRONamingMapper) getRegistrationInfo(criteria jobs.Criteria) FirstRegistration {
