...
func (b URLBuilder) GetPageURL(pageNumber int) string {
	...
	url := fmt.Sprintf("https://www.autoscout24.ro/lst/%s/%s/%s?atype=C&cy=%s&desc=0&regfrom=%d&kmto=%d&page=%d&powertype=kw&sort=standard&source=detailsearch&ustate=%s", brand, model, fuel, cyParam, *b.criteria.YearFrom, *b.criteria.KmTo, pageNumber, ustateParam)
	return url
}
